...
facetT *qh_findbest(qhT *qh, pointT *point, facetT *startfacet,
  ...
  int numpartnew=0;
  bool testhorizon= true; /* needed if precise, e.g., rbox c D6 | qhull Q0 Tv */
  ...
  design:
    for each horizon facet of coplanar best facets
      ...
      unless upperdelaunay or clearly outside
         update best facet
*/
facetT *qh_findbesthorizon(qhT *qh, bool ischeckmax, pointT* point, facetT *startfacet, bool noupper, realT *bestdist, int *numpart) {
...
facetT *qh_findbestnew(qhT *qh, pointT *point, facetT *startfacet,
  ...
  bool isdistoutside; /* true if distoutside is defined */
  bool testhorizon= true; /* needed if precise, e.g., rbox c D6 | qhull Q0 Tv */
...
*/
void qh_backnormal(qhT *qh, realT **rows, int numrow, int numcol, bool sign,
        coordT *normal, bool *nearzero) {
  int i, j;
