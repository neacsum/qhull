...
*/
bool qh_addfacetvertex(qhT *qh, facetT *facet, vertexT *newvertex) {
  vertexT *vertex;
...
void qh_check_bestdist(qhT *qh) {
  bool waserror= false, unassigned;
...
void qh_checkconvex(qhT *qh, facetT *facetlist, int fault) {
  ...
  realT dist;
  pointT *centrum;
  bool waserror= false, centrum_warning= false, tempcentrum= false, first_nonsimplicial= false, tested_simplicial, allsimplicial;
...
*/
void qh_checkfacet(qhT *qh, facetT *facet, bool newmerge, bool *waserrorp) {
...
*/
bool qh_checklists(qhT *qh, facetT *facetlist) {
  facetT *facet, *errorfacet= NULL, *errorfacet2= NULL, *previousfacet;
  vertexT *vertex, *vertexlist, *previousvertex, *errorvertex= NULL;
  bool waserror= false, newseen= false, nextseen= false, newvertexseen= false, visibleseen= false;
...
void qh_checkpolygon(qhT *qh, facetT *facetlist) {
  ...
  bool waserror= false, newseen= false, newvertexseen= false, nextseen= false, visibleseen= false;
  bool checkfacet;
...
*/
void qh_checkvertex(qhT *qh, vertexT *vertex, bool allchecks, bool *waserrorp) {
  bool waserror= false;
...
*/
facetT *qh_findbestfacet(qhT *qh, pointT *point, bool bestoutside,
...
*/
facetT *qh_findfacet_all(qhT *qh, pointT *point, bool noupper, realT *bestdist, bool *isoutside,
                          int *numpart) {
...
coordT qh_matchdupridge(qhT *qh, facetT *atfacet, int atskip, int hashsize, int *hashcount) {
  bool same, ismatch, isduplicate= false;
  int hash, scan;
    ...
    for (newfacet=atfacet, newskip=atskip; newfacet; newfacet= nextfacet, newskip= nextskip) {
      ...
      for (scan=hash; (facet= SETelemt_(qh->hash_table, scan, facetT));
        ...
        if (qh_matchvertices(qh, 1, newfacet->vertices, newskip, facet->vertices, &skip, &same)) {
          ...
          ismatch= (same == (bool)(newfacet->toporient ^ facet->toporient));
...
*/
void qh_resetlists(qhT *qh, bool stats, bool resetVisible /* qh.newvertex_list newfacet_list visible_list */) {
  vertexT *vertex;
...
*/
bool qh_vertexsubset(setT *vertexsetA, setT *vertexsetB) {
