...
*/
void qh_all_merges(qhT *qh, bool othermerge, bool vneighbors) {
  facetT *facet1, *facet2, *newfacet;
  mergeT *merge;
  bool wasmerge= false, isreduce;
...
*/
bool qh_checkzero(qhT *qh, bool testall) {
...
*/
void qh_findbest_test(qhT *qh, bool testcentrum, facetT *facet, facetT *neighbor,
...
facetT *qh_findbestneighbor(qhT *qh, facetT *facet, realT *distp, realT *mindistp, realT *maxdistp) {
  ...
  ridgeT *ridge, **ridgep;
  bool nonconvex= true, testcentrum= false;
...
*/
void qh_flippedmerges(qhT *qh, facetT *facetlist, bool *wasmerge) {
...
*/
void qh_forcedmerges(qhT *qh, bool *wasmerge) {
...
*/
bool qh_getpinchedmerges(qhT *qh, vertexT *apex, coordT maxdupdist, bool *iscoplanar /* qh.newfacet_list, qh.vertex_mergeset */) {
  ...
  FOREACHneighbor_i_(qh, facet) {
    if (neighbor == qh_MERGEridge)
      ...
      ridge->vertices= qh_setnew_delnthsorted(qh, facet->vertices, qh->hull_dim,
                                                          neighbor_i, 0);
      toporient= (bool)(facet->toporient ^ (neighbor_i & 0x1));
      if (toporient) {
        ridge->top= facet;
...
*/
void qh_mark_dupridges(qhT *qh, facetT *facetlist, bool allmerges) {
  facetT *facet, *neighbor, **neighborp;
  int nummerge=0;
  mergeT *merge, **mergep;
...
*/
void qh_mergecycle_all(qhT *qh, facetT *facetlist, bool *wasmerge) {
    ...
    FOREACHneighbor_i_(qh, same) {       /* note: !newfact->simplicial */
      if (neighbor->visitid != samevisitid && neighbor->simplicial) {
        ...
        ridge->vertices= qh_setnew_delnthsorted(qh, same->vertices, qh->hull_dim,
                                                          neighbor_i, 0);
        toporient= (bool)(same->toporient ^ (neighbor_i & 0x1));
        if (toporient) {
          ridge->top= newfacet;
          ridge->bottom= neighbor;
          ridge->simplicialbot= true;
...
*/
void qh_mergefacet(qhT *qh, facetT *facet1, facetT *facet2, mergeType mergetype, realT *mindist, realT *maxdist, bool mergeapex) {
  bool traceonce= false;
  vertexT *vertex, **vertexp;
...
*/
void qh_mergesimplex(qhT *qh, facetT *facet1, facetT *facet2, bool mergeapex) {
    ...
    for each vertex in facet
      if vertex not in a ridge (i.e., no longer used)
        ...
        unless vertex in another facet
          add vertex to qh.del_vertices for later deletion
*/
bool qh_remove_extravertices(qhT *qh, facetT *facet) {
  ridgeT *ridge, **ridgep;
  vertexT *vertex, **vertexp;
  bool foundrem= false;
...
*/
bool qh_renameridgevertex(qhT *qh, ridgeT *ridge, vertexT *oldvertex, vertexT *newvertex) {
  int nth= 0, oldnth;
  facetT *temp;
  vertexT *vertex, **vertexp;
...
*/
bool qh_test_appendmerge(qhT *qh, facetT *facet, facetT *neighbor, bool simplicial) {
  realT angle= -REALmax;
  bool okangle= false;
...
*/
bool qh_test_centrum_merge(qhT *qh, facetT *facet, facetT *neighbor, realT angle, bool okangle) {
  coordT dist, dist2, mergedist;
  bool isconcave= false, iscoplanar= false;
...
*/
bool qh_test_nonsimplicial_merge(qhT *qh, facetT *facet, facetT *neighbor, realT angle, bool okangle) {
  ...
  bool isconcave= false, iscoplanar= false, maybeconvex= false, isredundant= false;
...
*/
bool qh_test_vneighbors(qhT *qh /* qh.newfacet_list */) {
...
*/
setT *qh_vertexridges(qhT *qh, vertexT *vertex, bool allneighbors) {
  facetT *neighbor, **neighborp;
