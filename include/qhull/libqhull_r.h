...
/* include stat_r.h after defining bool.  Needed for qhT in libqhull_r.h */
...
*/
  ...
  bool GETarea;          /* true 'Fa', 'FA', 'FS', 'PAn', 'PFn' if compute facet area/Voronoi volume in io_r.c */
...
void    qh_freeqhull(qhT *qh, bool allmem);
void    qh_init_A(qhT *qh, FILE *infile, FILE *outfile, FILE *errfile, int argc, char *argv[]);
void    qh_init_B(qhT *qh, coordT *points, int numpoints, int dim, bool ismalloc);
...
void    qh_initqhull_globals(qhT *qh, coordT *points, int numpoints, int dim, bool ismalloc);
